...
# time after which flow is "quasi-steady / periodic" for averaging
t_start_avg = 0.015   # <-- change e.g. to 10.0 once you know when transients end
...
def get_time_dirs(base):
    """Return sorted list of (t, path) for all numeric time dirs under base."""
    times = []
    for name in os.listdir(base):
...
# -------------------------------------------------
# 2) Time-averaged profiles: inlet / internal locations
#    Centerline *_Ux* profiles:
#      - excluded from plots
#      - used only for printing values and Re_cyl
# -------------------------------------------------
def plot_profiles_and_centerline(profiles_root, t_start):
    # collect time directories
    all_times = get_time_dirs(profiles_root)
    if not all_times:
        print(f"[Warning] No time directories in {profiles_root}")
        return None, None

    # keep only times >= t_start_avg
    time_dirs = [(t, p) for (t, p) in all_times if t >= t_start]
    if not time_dirs:
        print(f"[Warning] No time directories >= {t_start} s in {profiles_root}")
        return None, None

    print(f"\nUsing profile data time-averaged over t >= {t_start} s")
    print(f"  Number of time directories used: {len(time_dirs)}")

    # Discover file names from the first valid time directory
    _, first_dir = time_dirs[0]
    U_files = []
    T_files = []
    for name in sorted(os.listdir(first_dir)):
        if name.endswith(".xy"):
            if "_U" in name:
                U_files.append(name)
            elif "_T" in name:
    ...
    if not U_files and not T_files:
        print("[Warning] No *.xy profile files found in time directories.")
        return None, None

    # Prepare accumulators: filename -> (coord_ref, sum_values, count)
    def init_accumulators(file_list):
        acc = {}
        for fname in file_list:
            acc[fname] = {"coord": None, "sum": None, "count": 0}
        return acc

    U_acc = init_accumulators(U_files)
    T_acc = init_accumulators(T_files)

    # Loop over times and accumulate
    for t, tdir in time_dirs:
        for fname, acc in U_acc.items():
            ...
            coord = data[:, 0]
            vals  = data[:, 1]
            # initialise coord and sum
            if acc["coord"] is None:
                acc["coord"] = coord
                acc["sum"]   = np.zeros_like(vals)
            else:
                # assume same coord; if not, could interpolate here
                if len(coord) != len(acc["coord"]):
                    print(f"[Warning] Inconsistent coord length for {fname} at time {t}, skipping this file.")
                    continue
            acc["sum"]   += vals
            acc["count"] += 1

        for fname, acc in T_acc.items():
            path = os.path.join(tdir, fname)
            if not os.path.isfile(path):
                continue
            data = load_xy(path)
    ...
    # Convert accumulators to mean profiles
    def make_means(acc_dict, field_name):
        profiles = {}
        for fname, acc in acc_dict.items():
            if acc["count"] == 0 or acc["coord"] is None:
                print(f"[Warning] No samples for {field_name} file {fname}, skipping.")
                continue
            mean_vals = acc["sum"] / acc["count"]
            profiles[fname] = (acc["coord"], mean_vals)
        return profiles

    U_profiles = make_means(U_acc, "U")
    T_profiles = make_means(T_acc, "T")

    # --- Plot all velocity profiles (excluding centerline) ---
    if U_profiles:
        plt.figure()
        for fname, (coord, u) in U_profiles.items():
        ...
        plt.grid(True, linestyle="--", linewidth=0.7)
        plt.ylabel("y [m]")
        plt.xlabel(r"$\langle U \rangle_t$ [m/s]")
        plt.title("Velocity profiles (time-averaged)")
        plt.legend()
        plt.tight_layout()
        plt.savefig("profiles_U_timeavg.png", bbox_inches="tight")
        plt.savefig("profiles_U_timeavg.pdf", bbox_inches="tight")

    # --- Plot all temperature profiles (excluding centerline) ---
    if T_profiles:
        plt.figure()
        for fname, (coord, T) in T_profiles.items():
            if "centerline" in fname.lower():
                continue
            label = fname.replace(".xy", "")
            plt.plot(T, coord, label=label)
        plt.grid(True, linestyle="--", linewidth=0.7)
        plt.xlabel(r"$\langle T \rangle_t$ [K]")
        plt.ylabel("y [m]")
        plt.title("Temperature profiles (time-averaged)")
        plt.legend()
        plt.tight_layout()
        plt.savefig("profiles_T_timeavg.png", bbox_inches="tight")
        plt.savefig("profiles_T_timeavg.pdf", bbox_inches="tight")

    # --- Centerline values at various x positions (time-averaged) ---
    center_U_files = [f for f in U_profiles.keys() if "centerline" in f.lower()]
    center_T_files = [f for f in T_profiles.keys() if "centerline" in f.lower()]

    if not center_U_files:
        print("\n[Info] No 'centerline' profile files found – cannot report centerline values.")
        return None, None

    # use the first centerline U profile
    fnameU = center_U_files[0]
    x_center, U_centerline = U_profiles[fnameU]

    # Optional temperature along same centerline
    T_centerline = None
    if center_T_files:
        fnameT = center_T_files[0]
        x_center_T, T_centerline = T_profiles[fnameT]
    ...
    print("\nTime-averaged centerline values U(x), T(x) at selected positions (from inlet):")
    ...
    for x_rel_mm, x_abs in zip(centerline_x_rel_mm, x_targets):
        ...
        if T_centerline is not None:
            ...
            print(f"  x_rel = {x_rel_mm:5.1f} mm (x = {x_abs: .4f} m):  "
                  f"<U>_t = {U_val:.6g} m/s,  <T>_t = {T_val:.6g} K")
        else:
            print(f"  x_rel = {x_rel_mm:5.1f} mm (x = {x_abs: .4f} m):  "
                  f"<U>_t = {U_val:.6g} m/s")
    ...
    if (x_abs_for_Re_cyl >= x_center.min()) and (x_abs_for_Re_cyl <= x_center.max()):
        ...
        print("\nReynolds number at cylinder (time-averaged centerline, "
              "x_rel = 100 mm, x = -0.05 m):")
...
if __name__ == "__main__":
    ...
    # 3) Time-averaged profiles & Re_cyl from centerline
    profiles_root = os.path.join(post_dir, "profiles")
    if os.path.isdir(profiles_root):
        _, Re_cyl = plot_profiles_and_centerline(profiles_root, t_start_avg)
    else:
    ...
    print("  profiles_U_timeavg.[png/pdf]")
    print("  profiles_T_timeavg.[png/pdf]")
    print("\nDone.\n")

    # 4) Cell count from log file (optional but useful for mesh plot)
